...
void test_encrypt_dig() {
    ...
    unsigned char dest[2048], fdest[4096];
    ...
    cjks_sha1(fdest + SHA_DIGEST_LENGTH + dlen, 2, password, (size_t)plen, dest, (size_t)dlen);

    cjks_b64_print(fdest, SHA_DIGEST_LENGTH + dlen + SHA_DIGEST_LENGTH);
}
