...
const uchar b64password[] = "AGMAaABhAG4AZwBlAGkAdA==";
...
void test_decrpyt_dig() {
    ...
    char password[128];
    int plen = cjks_b64decode((uchar*)password, b64password, sizeof(b64password) - 1);

    uchar dest[2048], b64dest[4096];
...
void test_encrypt_dig() {
    const uchar b64iv[] = "24Zy9qgZlnJBNMMDpOEXDIEBJas=";
    ...
    uchar dest[2048], fdest[4096], b64fdest[4096];
    ...
    uchar sigbufdump[4096], * sigbufptr = sigbufdump;

    i2d_X509_SIG(sig, &sigbufptr);

    uchar b64sigbuf[4096];
